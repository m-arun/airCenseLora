...
#define PMITERATION 				10
uint8_t flag = 0;
uint16_t uartMsgLen = 0;


uint8_t rxBuffer[150];
uint8_t *token;

float temperature, NO2, CO, humidity, PM2_5, PM10=0;
uint32_t CO2;
...
static void PrepareTxFrame(uint8_t port) {
	...
	case 3:
		{
			airCenseProto air = {NO2, CO, temperature, humidity, CO2, PM2_5, PM10};
//			airCenseProto air = {NO2, CO, humidity, CO2, PM10};
			pb_ostream_t stream = pb_ostream_from_buffer(buffer, sizeof(buffer));
			pb_encode(&stream, airCenseProto_fields, &air);
...
int main(void) {
	...
	while (1) {
		if ( flag == 1){
			flag = 0;
			UartGetBuffer(&Uart1, rxBuffer, 150, &uartMsgLen);

			token = strtok(rxBuffer,",");
			NO2 = (float) atof(token);
			token = strtok(NULL,",");
			CO = atof(token);
			token = strtok(NULL,",");
